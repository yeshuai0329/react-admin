...
interface IOptions {
  openModal: (title?: string | undefined, record?: RolesRecord | undefined) => void, // 新建编辑模态框
...
export const useAboutRolesTable = ({
  ...
  const editRoleStatus = (val: boolean, record: RolesRecord) => {
    Modal.confirm({
      ...
      content: val ? `确定启用 ${record.rolesName} 角色 ?` : `确定禁用 ${record.rolesName} 角色 ?`,
      okText: '确认',
      cancelText: '取消',
      onOk: () => {
        console.log(`obj`, 11)
  ...
  const rolesColumns : ColumnsType<RolesRecord> = [
    ...
    },
    {
      ...
      align: 'center',
      width: 200,
      render: function rolesStatusRender(text: number, record: RolesRecord, index: number) {
        return (
          <Switch
        ...
        return (
          <Space>
            <AuthButton
              type='link'
              auth='ROLES_EDIT'
              onClick={() => { openModal(SECOND_TYPE, record) }}
            >
              <EditOutlined />编辑
            </AuthButton>
            <AuthButton
